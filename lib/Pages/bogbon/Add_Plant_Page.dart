...
class _AddPlantPageState extends State<AddPlantPage> {
  ...
  Future<void> _savePlant() async {
    if (_formKey.currentState!.validate()) {
      ...
      final wateringDays = int.tryParse(_wateringController.text) ?? 7;

      final plant = PlantModel(
        id: plantId,
        name: _nameController.text.trim(),
        latinName: widget.editPlant?.latinName ?? "",
        family: widget.editPlant?.family ?? "",
        category: _categoryController.text.trim(),
        origin: widget.editPlant?.origin ?? [],
        ...
        matureSize: widget.editPlant?.matureSize ?? const MatureSizeModel(heightCm: 0, widthCm: 0),
        lifespan: widget.editPlant?.lifespan ?? "",
        care: CareModel(
          watering: WateringModel(
            days: wateringDays,
            summerDays: widget.editPlant?.care.watering.summerDays ?? wateringDays,
            winterDays: widget.editPlant?.care.watering.winterDays ?? wateringDays * 2,
            amountMl: widget.editPlant?.care.watering.amountMl ?? 200,
            method: widget.editPlant?.care.watering.method ?? "Tuproq qurigandan keyin sug‘orish",
            overwateringRisk: widget.editPlant?.care.watering.overwateringRisk ?? "medium",
          ),
          sunlight: widget.editPlant?.care.sunlight ?? const SunlightModel(type: SunlightType.partial, hours: 4, luxMin: 0, luxMax: 0),
          ...
          humidity: widget.editPlant?.care.humidity ?? const HumidityModel(min: 30, ideal: 50, max: 70),
          soil: widget.editPlant?.care.soil ?? const SoilModel(type: "Universal", phMin: 6.0, phMax: 7.0, drainage: "good"),
          fertilizer: widget.editPlant?.care.fertilizer ?? const FertilizerModel(type: "Universal", npk: "10-10-10", frequencyDays: 30),
        ),
        seasonalCare: widget.editPlant?.seasonalCare ?? const SeasonalCareModel(spring: [], summer: [], autumn: [], winter: []),
        benefits: widget.editPlant?.benefits ?? [],
        tips: widget.editPlant?.tips ?? [],
        commonProblems: widget.editPlant?.commonProblems ?? [],
        diseases: widget.editPlant?.diseases ?? [],
        pests: widget.editPlant?.pests ?? [],
        propagationMethods: widget.editPlant?.propagationMethods ?? [],
        companionPlants: widget.editPlant?.companionPlants ?? [],
        isToxicForPets: widget.editPlant?.isToxicForPets ?? false,
        isToxicForChildren: widget.editPlant?.isToxicForChildren ?? false,
        flowering: widget.editPlant?.flowering ?? const FloweringModel(season: "", flowerColor: "", floweringAgeYears: 0),
        ...
        aiContext: widget.editPlant?.aiContext ?? const AiContextModel(plantType: "", wateringSensitivity: "", droughtTolerance: "", diseaseRisk: "", petFriendly: false),
